          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="col-span-2">
              ...
              <div className="relative">
                ...
                <input required className="w-full bg-darkBg border border-gray-800 rounded-xl py-3 pl-12 pr-4 text-white focus:border-neon outline-none" placeholder="Ex: Aspen Mountain - Casa Alma" value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type="number" step="0.01" required className="w-full bg-darkBg border border-gray-800 rounded-xl py-3 pl-12 pr-4 text-white focus:border-neon outline-none" placeholder="0.00" value={formData.price || ''} onChange={e => setFormData({...formData, price: parseFloat(e.target.value)})} />
              </div>
            </div>
            ...
            <div className="col-span-2 p-5 bg-darkBg/50 border border-gray-800 rounded-2xl grid grid-cols-2 sm:grid-cols-4 gap-6">
               ...
               <div>
                  ...
                  <input type="number" className="w-full bg-cardBg border border-gray-700 rounded-lg p-2 text-white outline-none" value={formData.capacity} onChange={e => setFormData({...formData, capacity: parseInt(e.target.value)})} />
               </div>
               <div>
                  ...
                  <input type="number" className="w-full bg-cardBg border border-gray-700 rounded-lg p-2 text-white outline-none" value={formData.area} onChange={e => setFormData({...formData, area: parseFloat(e.target.value)})} />
               </div>
            </div>
            ...
            <div className="col-span-2 p-6 bg-neon/5 border border-neon/10 rounded-2xl">
              ...
              <div className="relative">
                ...
                <input type="number" step="0.01" required className="w-full bg-darkBg border border-neon/30 rounded-xl py-3 pl-12 pr-4 text-white focus:border-neon outline-none font-bold" placeholder={formData.commissionType === 'percentage' ? "0.00%" : "R$ 0,00"} value={formData.commissionValue || ''} onChange={e => setFormData({...formData, commissionValue: parseFloat(e.target.value)})} />
              </div>
            </div>
          </div>
        </form>

        <div className="p-6 border-t border-gray-800 bg-cardBg grid grid-cols-2 gap-4">
          ...
          <button type="submit" onClick={handleSubmit} className="py-4 bg-neon text-darkBg font-black rounded-xl hover:bg-neon/90 transition-all shadow-[0_0_20px_rgba(124,255,79,0.2)]">Salvar Produto</button>
        </div>
      </div>
    </div>
  );
