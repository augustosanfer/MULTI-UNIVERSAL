  ...
  return (
    <div className="max-w-7xl mx-auto p-6 md:p-10 pb-20">
      ...
      {isLoading ? (
        ...
        <div className="bg-cardBg/50 border-2 border-dashed border-gray-800 rounded-3xl p-20 text-center">
          <Home size={48} className="mx-auto text-gray-700 mb-4 opacity-20" />
          <p className="text-gray-500">Nenhum produto cadastrado ainda.</p>
          <button onClick={() => setShowForm(true)} className="text-neon font-bold mt-2 hover:underline">Começar agora</button>
        </div>
      ) : (
            ...
            <div key={product.id} className="bg-cardBg border border-gray-800 rounded-[2rem] overflow-hidden group hover:border-neon/30 transition-all shadow-xl flex flex-col">
              <div className="relative h-64 overflow-hidden">
                ...
                <div className="absolute bottom-4 left-4 right-4 flex justify-between items-center bg-darkBg/60 backdrop-blur-md p-2 rounded-xl border border-white/10 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={() => { setEditingProduct(product); setShowForm(true); }} className="p-2 text-white hover:text-neon"><Edit size={18}/></button>
                  <button onClick={() => handleDelete(product.id)} className="p-2 text-white hover:text-red-500"><Trash2 size={18}/></button>
                </div>
              </div>
              ...
              <div className="p-6 flex-1 flex flex-col">
                <div className="mb-4">
                  ...
                  <div className="flex items-center gap-1.5 text-gray-500 text-sm">
                    <MapPin size={14} className="text-neon" /> {product.location}
                  </div>
                </div>
                ...
                <div className="mb-6">
                  ...
                  <div className="mt-1 flex items-center gap-2">
                    ...
                    <span className="text-xs font-black text-white bg-white/5 px-2 py-0.5 rounded border border-white/10">
                      {product.commissionType === 'percentage' ? `${product.commissionValue}%` : formatCurrency(product.commissionValue)}
                    </span>
                  </div>
                </div>
